...
# ---------------------------------------------------------------------------
# Rentabilidad desde la fecha de predicción (live data de Yahoo)
# ---------------------------------------------------------------------------
st.subheader("📈 Rentabilidad desde la predicción (live, Yahoo Finance)")

start_date = (input_month + pd.offsets.BDay(1)).strftime("%Y-%m-%d")
today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")

tickers_to_fetch = [ASSET_PROXY_TICKERS[a] for a in active_weights if a in ASSET_PROXY_TICKERS]

if not tickers_to_fetch:
    st.warning("No hay activos en el portfolio con proxy ticker en Yahoo Finance.")
else:
    with st.spinner(f"Descargando precios desde {start_date} ..."):
        prices = fetch_asset_prices(tickers_to_fetch, start_date, today_str)

    if prices.empty:
        st.warning("Yahoo Finance no devolvió datos para el rango pedido.")
    else:
        rebased = prices.div(prices.iloc[0]) * 100
        rev_map = {v: k for k, v in ASSET_PROXY_TICKERS.items()}
        rebased_named = rebased.rename(columns=rev_map)

        portfolio_weights_series = pd.Series({
            a: active_weights[a] for a in rebased_named.columns if a in active_weights
        })
        portfolio_weights_series = portfolio_weights_series / portfolio_weights_series.sum()
        portfolio_value = (rebased_named * portfolio_weights_series).sum(axis=1)

        try:
            spy = fetch_asset_prices(["SPY"], start_date, today_str)
            spy_rebased = (spy["SPY"] / spy["SPY"].iloc[0]) * 100
        except Exception:
            spy_rebased = None
        ...
        fig_eq.add_trace(go.Scatter(
            ...
            mode="lines", name="Portfolio TFM",
            line=dict(width=3, color="#1f77b4"),
        ))
        if spy_rebased is not None:
            fig_eq.add_trace(go.Scatter(
                x=spy_rebased.index, y=spy_rebased.values,
                mode="lines", name="Benchmark SP500 (SPY)",
                line=dict(width=2, color="#888", dash="dash"),
            ))
        fig_eq.update_layout(
            title=f"Equity curve desde {start_date} (rebase 100)",
            xaxis_title="Fecha", yaxis_title="Índice (100 = inicio)",
            height=420, hovermode="x unified",
        )
        st.plotly_chart(fig_eq, use_container_width=True)

        c1, c2, c3, c4 = st.columns(4)
        port_ret = portfolio_value.iloc[-1] / 100 - 1
        port_vol = portfolio_value.pct_change().std() * (252 ** 0.5)
        port_sharpe = (port_ret * (252 / len(portfolio_value))) / port_vol if port_vol > 0 else float("nan")
        max_dd = (portfolio_value / portfolio_value.cummax() - 1).min()

        c1.metric("Retorno total", f"{port_ret*100:+.2f}%")
        c2.metric("Volatilidad anual", f"{port_vol*100:.1f}%")
        c3.metric("Sharpe anualizado", f"{port_sharpe:.2f}")
        c4.metric("Max drawdown", f"{max_dd*100:.2f}%")

        st.markdown("**Rentabilidad por activo desde la predicción:**")
        last_returns = (rebased_named.iloc[-1] / 100 - 1) * 100
        per_asset = pd.DataFrame({
            "Activo":        last_returns.index,
            "Peso":          [f"{active_weights[a]:.1%}" for a in last_returns.index],
            "Ticker proxy":  [ASSET_PROXY_TICKERS[a] for a in last_returns.index],
            "Retorno %":     last_returns.values.round(2),
        }).sort_values("Retorno %", ascending=False)
        st.dataframe(per_asset, hide_index=True, use_container_width=True)

        st.caption(
            "📌 Caveat: los tickers usados aquí son ETFs proxy de cada asset class "
            "(SPY ≈ SP500, GLD ≈ gold, etc.), no exactamente los instrumentos sintéticos "
            "TR del modelo. Para visualización en streaming es suficiente; "
            "el backtest oficial del TFM usa los parquets de S3."
        )
...
# ---------------------------------------------------------------------------
st.markdown("---")
with st.expander("🔍 Metadata completa de la predicción"):
    st.json(prediction)

st.caption(
    f"Predicción generada el {generated_at.strftime('%Y-%m-%d %H:%M UTC')}. "
    f"Dashboard refrescado al cargar — usa el botón de Refrescar para invalidar cache."
)
